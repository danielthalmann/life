...
public class VisionCone : MonoBehaviour
{
...

    {
        ...
        float Currentangle = -fov.angle / 2;
        float angleIcrement = fov.angle / (VisionConeResolution - 1);
        float Sine;
        float Cosine;
...

        {
            ...
            if (Physics.Raycast(transform.position, RaycastDirection, out RaycastHit hit, fov.radius, fov.obstructionMask))
            {
                Vertices[i + 1] = VertForward * hit.distance;
            }
            else
            {
                Vertices[i + 1] = VertForward * fov.radius;
            }
